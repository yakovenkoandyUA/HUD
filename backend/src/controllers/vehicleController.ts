...
export async function updateVehicleProfile(req: Request, res: Response): Promise<void> {
  try {
    ...
    const allowed = ['make','model','year','plateNumber','vin','currentMileage','fuelType','purchaseDate','photoUrl','nextServiceMileage'] as const
    if (!space.vehicleProfile) {
      space.vehicleProfile = { make:'', model:'', year:null, plateNumber:'', vin:'', currentMileage:null, fuelType:'', purchaseDate:null, photoUrl:'', nextServiceMileage:null }
    }
    allowed.forEach(key => {
