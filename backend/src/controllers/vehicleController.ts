...
export async function updateVehicleProfile(req: Request, res: Response): Promise<void> {
  try {
    ...
    const allowed = ['make','model','year','plateNumber','vin','currentMileage','fuelType','purchaseDate','photoUrl'] as const
    if (!space.vehicleProfile) {
      space.vehicleProfile = { make:'', model:'', year:null, plateNumber:'', vin:'', currentMileage:null, fuelType:'', purchaseDate:null, photoUrl:'' }
    }
    allowed.forEach(key => {
