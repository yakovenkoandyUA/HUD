...
### ✅ Чек-ліст квестів: вкладеність (3 рівні) + призначення на сімейного юзера (2026-09-18)

Юзерський запит: у квесті з великим чек-лістом (напр. "Магазин") потрібно (1) позначати які пункти для кого з сімʼї, коли квест призначено на кількох людей, і (2) вкладені підпункти до 3 рівнів (Паста/Піца/Суп → кожен зі своїми кроками).

**Модель `ChecklistItem`** (`shared/types/index.ts`, `SprintTask.ts` на бекенді) отримав `parentId?: string | null` (вкладеність) і `assigneeId?: string | null` (призначення). Зберігання й раніше було `Schema.Types.Mixed` — зміни не вимагали міграції, але Zod-схема `updateTaskSchema.checklist` (`backend/src/validation/schemas.ts`) мала строгий `z.object({id,title,done})` без passthrough — нові поля тихо обрізались би при кожному PATCH. Пофіксено разом.

**Вкладеність** — кнопка "+" на кожному пункті (ховається на 3-му рівні), інпут для нового підпункту зʼявляється в кінці підсписку батька (після останнього наявного підпункту), не одразу під батьком. Рендер `TaskDetailModal` — depth-first обхід (`checklistRows`) замість плаского сорту "виконане вниз", щоб не розбивати ієрархію; сортування "виконане вниз" тепер працює всередині кожної групи siblings. Батьківський пункт автоматично стає `done`, коли всі діти `done` (чекбокс батька неактивний, стан похідний) — каскадно на всі рівні через `withAutoParents()` в `sprintStore.ts`. Видалення батька каскадно видаляє нащадків.

**Призначення на юзера** — маленький бейдж-аватарка на рядку чек-ліста (циклічний тап через людей, вже призначених на весь квест у "Виконавці"; показується тільки якщо квест поділений). `setChecklistAssignee` — новий action в `sprintStore.ts`.

Прогрес-бар/лічильник (`2/5`) рахують тільки "листові" пункти (без груп-заголовків), щоб не задвоювати прогрес при авто-завершенні батька — те саме виправлено в прев'ю `TaskCard`.
