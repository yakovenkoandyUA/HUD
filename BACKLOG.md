...
**Відкриті ідеї (не реалізовано, на узгодження):** "з ким" тег прямо на Memory (не лише через Plan→convert); тривалість на trip-картках замість однієї дати ("5 днів, Одеса"); історична погода на дату спогаду (wttr.in past dates); stats sheet по кількості/фото/місцям (segments вже рахуються).
