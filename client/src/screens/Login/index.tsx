...
const LoginScreen: React.FC = () => {
  ...
  const handleGoogleCredential = useCallback(async (response: GoogleCredentialResponse) => {
    ...
    try {
      ...
      const { token, user } = await res.json() as { token: string; user: { id: string; name: string; username: string; email?: string; avatarUrl: string | null; role: 'admin' | 'user'; f1Enabled: boolean; hasPIN: boolean } }
