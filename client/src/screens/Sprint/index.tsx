    ...
    <div className={styles.screen}>
      ...
      <div className={styles.tabBar}>
        <div className={styles.tabs}>
          <button
            className={`${styles.tab} ${tab === 'sprint' ? styles.tabActive : ''}`}
      ...
      <div className={styles.content}>
          ...
          <>
            ...
            {weekTasks.length === 0 ? (
              <div className={styles.emptyState}>
                ...
                <span className={styles.emptyTitle}>Тиждень чистий</span>
                <span className={styles.emptyHint}>Додай перше завдання</span>
              </div>
            ) : (
        ...
        {tab === 'lessons' && (
          <>
            ...
            {lessons.length === 0 && <p className={styles.emptySimple}>Уроків ще немає</p>}
          </>
        )}
          ...
          <>
            ...
            {sortedTodos.length === 0 ? (
              <div className={styles.emptyState}>
                ...
                <span className={styles.emptyTitle}>Список чистий. Поки що.</span>
              </div>
            ) : (
              <ul className={styles.list}>
                {sortedTodos.map(t => (
      ...
      <Modal isOpen={showAddTodo} onClose={() => setShowAddTodo(false)} title="Нова справа">
        <form onSubmit={handleAddTodo} className={styles.taskForm}>
          <div className={styles.modeRow}>
            <button
              type="button"
              className={`${styles.modeBtn} ${todoMode === 'single' ? styles.modeBtnActive : ''}`}
              onClick={() => setTodoMode('single')}
            >
              Одна річ
            </button>
            <button
              type="button"
              className={`${styles.modeBtn} ${todoMode === 'list' ? styles.modeBtnActive : ''}`}
              onClick={() => setTodoMode('list')}
            >
              Закупиться на повну
            </button>
          </div>
          <div className={styles.priorityRow}>
            {PRIORITIES.map(p => (
              <button
                key={p}
                type="button"
                className={`${styles.priBtn} ${todoPriority === p ? styles.priBtnActive : ''}`}
          ...
          {todoMode === 'single' ? (
            <input
              ...
              placeholder="Що треба зробити/купити?"
              autoFocus
            />
          ) : (
            <textarea
