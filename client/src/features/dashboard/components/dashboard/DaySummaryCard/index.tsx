  ...
  <div className={styles.grid}>
    ...
    <button type="button" className={styles.cell} onClick={onQuestsClick}>
      <div className={styles.cellHeader}>
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${styles.cellIcon} ${styles.cellIconAccent}`} aria-hidden="true">
          <circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/>
        </svg>
      </div>
      {activeQuests > 0 ? (
        <>
          <span className={`${styles.cellVal} ${styles.cellValAccent}`}>{activeQuests}</span>
          <span className={`${styles.cellSub} ${styles.cellSubAccent}`}>активних</span>
        </>
      ) : (
    ...
    <button type="button" className={styles.cell} onClick={onShoppingClick}>
      ...
      <span className={`${styles.cellSub} ${shoppingCount > 0 ? styles.cellSubAccent : styles.cellSubDim}`}>
    ...
    <button type="button" className={styles.cell} onClick={onMealsClick}>
      <div className={styles.cellHeader}>
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${styles.cellIcon} ${styles.cellIconSecond}`} aria-hidden="true">
          <path d="M3 2v7c0 1.1.9 2 2 2h0a2 2 0 0 0 2-2V2M5 11v11M21 2v20M21 2a5 5 0 0 0-5 5v4h5"/>
        </svg>
      </div>
      {meals.length > 0 ? (
        <>
          <span className={`${styles.cellVal} ${styles.cellValSecond}`}>{meals[0]}</span>
          {meals.length > 1 && (
            <span className={`${styles.cellSub} ${styles.cellSubDim}`}>+{meals.length - 1} страва</span>
          )}
        </>
      ) : (
        <>
          <span className={`${styles.cellVal} ${styles.cellValDim}`}>не заплановано</span>
          <span className={`${styles.cellSub} ${styles.cellSubAccent}`}>додати план</span>
        </>
      )}
    </button>
    ...
    <button type="button" className={styles.cell} onClick={onNotesClick}>
      <div className={styles.cellHeader}>
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${styles.cellIcon} ${styles.cellIconText}`} aria-hidden="true">
      ...
      {notesCount > 0 ? (
        <>
          ...
          <span className={`${styles.cellSub} ${styles.cellSubDim}`}>остання нотатка</span>
        </>
      ) : (
